...
export default function FaltanteReceiveInterface({ orderId, orderDisplayId, orderItems, manualMode = false }: Props) {
  ...
  async function handleManualReceive(lineItemId: string) {
    if (receivingItemId) return;
    setReceivingItemId(lineItemId);
  ...
  // === SCANNING / MANUAL: Interfaz principal ===
  return (
    <div className="print:hidden mt-4 space-y-3">
      {/* Header con progreso */}
      <div className="bg-yellow-600 text-white rounded-xl p-4">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs text-yellow-200 uppercase tracking-wider font-semibold">
            Recibir faltantes{manualMode ? ' — Modo manual' : ''}
          </span>
        </div>
      ...
      {/* Feedback del último recibido (modo manual) */}
      {manualMode && lastScannedName && !scanning && (
        <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center gap-2">
          <svg className="w-4 h-4 text-green-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
          </svg>
          <span className="text-green-700 text-sm truncate">{lastScannedName}</span>
        </div>
      )}
                ...
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  {!isDone && !mi.barcode && (
                    <button
                      ...
                      className="w-10 h-10 bg-yellow-500 text-white rounded-xl flex items-center justify-center text-xl font-bold active:bg-yellow-600 disabled:opacity-50 transition-colors shadow-sm"
                    >
                      {receivingItemId === mi.lineItemId ? (
                        <svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24">
                          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                        </svg>
