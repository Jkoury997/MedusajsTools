        ...
        <summary className="flex items-center justify-between px-4 py-3 cursor-pointer list-none">
          <div className="flex items-center gap-2 min-w-0">
            ...
            {(order.shipping_address?.phone || order.billing_address?.phone || order.customer?.phone) && (
              <span className="text-xs text-gray-500 hidden sm:inline">· {order.shipping_address?.phone || order.billing_address?.phone || order.customer?.phone}</span>
            )}
          </div>
          ...
          {(order.shipping_address?.phone || order.billing_address?.phone || order.customer?.phone) && (() => {
            ...
            return (
              <div className="flex items-center gap-2">
                ...
                <a
                  href={`https://wa.me/${formatWhatsAppNumber(phone!)}?text=Hola! Te escribimos de Marcela Koury por tu pedido %23${order.display_id}`}
                  target="_blank"
      ...
      <div className="hidden print:block text-xs border border-gray-400 p-2 mb-2">
        <div className="flex justify-between">
          <div>
            ...
            {(order.shipping_address?.phone || order.billing_address?.phone || order.customer?.phone) && <span className="ml-2">Tel: {order.shipping_address?.phone || order.billing_address?.phone || order.customer?.phone}</span>}
          </div>
